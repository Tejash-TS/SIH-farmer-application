			...
			<div class="card">
                <div class="card-header">
				  ...
				  <div class="card-tools">
					<a href="Add_chapter" class="btn bg-primary btn-sm" data-toggle="modal" data-target="#add_modal">Add disease</a>
				  </div>
				</div>
              ...
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped dataTable dtr-inline" aria-describedby="example1_info">
					...
					<?php
						$sr=1;
						$stml=$conn->prepare("CALL s_pr_get_all_diseases()");
						if($stml->execute())
						{
					...
					?>
									<tr>
										...
										<td>
											<button value="<?= ed("en", $res->diseases_id) ?>" class="view_btn btn btn-sm btn-primary m-1 ">View</button>
											<button value="<?= ed("en", $res->diseases_id) ?>" class="edit_btn btn btn-sm btn-info m-1 ">Edit</button>
											<button value="<?= ed("en", $res->diseases_id) ?>" class="delete_btn btn btn-sm btn-danger m-1 ">Delete</button>
										</td>
					...
					<form id="add_form">
						<div class="row">
							<div class="form-group col-sm-6">
								...
								<input type="text" class="form-control" name="disease_name" placeholder="Enter Disease Name" required>
							...
							<div class="form-group col-sm-6">
								...
								<input type="Text" class="form-control" name="one_line_description" placeholder="Enter one line description" required>
							...
							<div class="form-group col-sm-6">
								...
								<textarea class="form-control" name="causes" placeholder="Enter causes" required></textarea>
								<small id="mb_no_exist" class="form-text text-danger"></small>
							</div>
							<div class="form-group col-sm-6">
							...
							<div class="form-group col-sm-6">
								...
								<textarea class="form-control" name="prevention" placeholder="Enter prevention" required></textarea>
								<small id="mb_no_exist" class="form-text text-danger"></small>
							</div>
...
$(document).on('click', '.edit_btn', async function(event) {
	...
	if (await confirmation_alert("Do you really want to Edit this Disease? ")) 
	{
		var btn = $(this);
		var btn_value = btn.val();
		var formData = new FormData();
...
$(document).on('submit', '#update_form', async function(event) {
	...
	if (await confirmation_alert("Do you really want to Update this Disease? ")) 
	{
		var formData = new FormData(this);
		$.ajax({
			url: 'admin/disease_dao', 
...
$(document).on('click', '.delete_btn', async function(event) {
	...
	if (await delete_confirmation("Do you really want to delete this Disease? This action cannot be undone.")) 
	{
		var btn = $(this);
		var btn_value = btn.val();
		var formData = new FormData();
